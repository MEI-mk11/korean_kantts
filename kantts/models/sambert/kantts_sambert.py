...
class KanTtsSAMBERT(nn.Module):
    def __init__(self, config):
        ...
        if not self.se_enable:
            ...
            self.spk_tokenizer = nn.Embedding(8, config["speaker_units"])
    ...
    def forward(
        ...
        if self.MAS and is_training:
            ...
            # except:
            #     print(attn_priors.size())
            #     print(duration_targets.sum(dim=1))
            #     print(output_lengths)

            if not torch.all(torch.eq(duration_targets.sum(dim=1), output_lengths)):
                output_lengths = duration_targets.sum(dim=1)
